  ...
  module Baps
    ...
    module Commands
      ...
      class Initiate < Command
        ...
        #            to the BAPS server on which the response should be
        #            received.
        #
        # Yields the seed given by the BAPS server for this session.  This seed
        #   should be passed into an Authenticate command.  Note that this
        #   yield is indirect and may occur some time after the call.
        #
        # Examples
        #
      ...
      class Authenticate < Command
        ...
        #            received.
        #
        # Yields the error code (see Authenticate::Errors) returned by the
        #   server on authentication, as well as the server's human-readable
        #   response string.  An error code of Authenticate::Errors::OK
        #   signifies a successful authentication.  Note that this yield is
        #   indirect and may occur some time after this call.
        #
        # Returns nothing.
        def run(dispatch, queue)
      ...
      # the BAPS server.
      #
      # This command performs the same work as a chain of Initiate,
      # Authenticate, and Synchronise.
      class Login < Command
        ...
        #            to the BAPS server on which the response should be
        #            received.
        # block    - A block to execute upon the entire login process
        #            concluding.  See below.
        #
        # Yields, indirectly, the authentication status code and explanation
        #   string; see Authenticate#run for details.  Note that the yield
        #   occurs inside a block itself yielded by the dispatch, and thus may
        #   occur quite some time after this call.
